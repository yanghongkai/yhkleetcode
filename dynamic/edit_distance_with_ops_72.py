...
class Solution:
    def minDistance(self, word1, word2) -> int:
        ...
        m = len(word1)
        n = len(word2)
        dp = [[Node() for j in range(n + 1)] for i in range(m + 1)]
        # base case dp[0][..] = len(str2) add
        for j in range(1, n + 1):
            dp[0][j].val = j
            dp[0][j].op = 1  # add

        # ase case dp[...][0] = len(str1) delete
        ...
        for i in range(1, m + 1):
            for j in range(1, n + 1):
                if word1[i - 1] == word2[j - 1]:
                    dp[i][j].val = dp[i - 1][j - 1].val  # no op
                    dp[i][j].op = 0  # skip no op
                else:
                    replace = dp[i - 1][j - 1].val
                    add = dp[i][j - 1].val
                    delete = dp[i - 1][j].val
                    min_val = min([replace, add, delete])
                    dp[i][j].val = min_val + 1
                    op = 0
                    if add == min_val:
                        op = 1
                    if delete == min_val:
